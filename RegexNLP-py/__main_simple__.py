...
def run_variable(variable, settings, mode):
    ...
    for rule in cur_run:
        ...
        for cur_dataset in available_datasets:
            ...
            gen_path = os.path.join(cwd, "generated_data", rule_name, cur_dataset)
            respond(gen_path, debug=True)
...
def set_cwd(current_dir):
    ...
    global cwd
    cwd = current_dir
    respond("cwd set to " + cwd, debug=True)


"""
@exposed_function
def paths():
    respond({'cwd': os.getcwd(), '__file__': os.path.dirname(os.path.abspath(__file__))})
